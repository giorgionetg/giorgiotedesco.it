...
export default function VisuraKanban({ variantId }: VisuraKanbanProps) {
  ...
  useEffect(() => {
    if (!discountCode || !isRequestConfirmed) return;
  ...
  }, [discountCode, isRequestConfirmed]);
  ...
  const confirmRequest = async () => {
    ...
    try {
      ...
      trackVisuraExperiment('visura_start', variantId);
    } catch {
    ...
    <section id="avvia-visura" tabIndex={-1} className="scroll-mt-24 overflow-x-hidden border-y border-slate-200 bg-transparent py-12 outline-none sm:py-24">
      <div className="mx-auto w-full max-w-6xl px-5 sm:px-8">
        <div className="w-full lg:max-w-2xl">
          ...
          <h2 className="mt-4 text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">Un passaggio alla volta, con le informazioni che servono.</h2>
        </div>
        ...
        <div className="mt-8 grid w-full min-w-0 gap-4 md:mt-10 md:grid-cols-4">
            ...
            <AnimatePresence mode="wait">
              {step === 1 ? (
                <motion.form key="website-form" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} onSubmit={submitWebsite} className="mt-6">
                  <label htmlFor="website-availability" className="block text-sm font-medium text-slate-800">Come vuoi iniziare?</label>
                  <select id="website-availability" value={websiteAvailability} onChange={(event) => selectWebsiteAvailability(event.target.value)} className="select mt-2 w-full border-slate-300 bg-white text-slate-900 focus:border-brand-blue focus:outline-none">
                  ...
                  {websiteAvailability === 'has-site' && <div className="mt-5">
                    <label htmlFor="kanban-url" className="sr-only">Indirizzo del sito</label>
                    <div className={`flex items-center rounded-xl border bg-white p-1.5 ${urlError ? 'border-red-500' : 'border-slate-300 focus-within:border-brand-blue focus-within:ring-2 focus-within:ring-blue-100'}`}>
                      <input id="kanban-url" type="text" inputMode="url" autoComplete="url" value={websiteUrl} onChange={(event) => setWebsiteUrl(event.target.value)} placeholder="esempio.it" className="min-w-0 flex-1 bg-transparent px-3 py-2 text-lg text-slate-900 outline-none placeholder:text-slate-400" />
                    ...
                    <div className="mt-5 grid gap-4 sm:grid-cols-2 sm:items-stretch">
                      <div>
                        <label htmlFor="intervention-type" className="block text-sm font-medium text-slate-800">Intervento che stai valutando</label>
                        <select id="intervention-type" value={interventionType} onChange={(event) => setInterventionType(event.target.value)} className="select mt-2 w-full border-slate-300 bg-white text-slate-900 focus:border-brand-blue focus:outline-none">
                          <option value="" disabled>Seleziona un’opzione</option>
                      ...
                      <div className="flex flex-col">
                        <label htmlFor="intended-outcome" className="block text-sm font-medium text-slate-800">Cosa speri di ottenere? <span className="font-normal text-slate-500">(facoltativo)</span></label>
                        <textarea id="intended-outcome" value={intendedOutcome} onChange={(event) => setIntendedOutcome(event.target.value)} placeholder="Più richieste, messaggio più chiaro…" rows={4} className="textarea mt-2 min-h-32 w-full flex-1 resize-y border-slate-300 bg-white text-slate-900 placeholder:text-slate-400 focus:border-brand-blue focus:outline-none" />
                      </div>
                    </div>
                  </div>}
          ...
          <motion.article layout transition={{ type: 'spring', stiffness: 280, damping: 30 }} className={`${panelClass} ${step === 1 ? 'md:col-start-4 md:row-start-2' : step === 2 ? 'md:col-span-1' : 'md:col-start-2 md:row-span-2 md:col-span-3'} ${step < 2 ? 'opacity-60' : ''}`}>
            <div>
              ...
              <h3 className="mt-3 text-xl font-bold text-slate-900">{isRequestConfirmed ? 'Grazie. Riceverai un aggiornamento entro 72 ore dalla conferma.' : 'Conferma per avviare la diagnosi'}</h3>
            </div>
            <AnimatePresence mode="wait">
              {step === 3 ? (
                <motion.div key="discount-code" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-6">
                  {!isRequestConfirmed && <p className="text-sm leading-relaxed text-slate-600">Nessuna diagnosi è stata ancora avviata. Per procedere, autorizza il trattamento necessario alla richiesta e invia la conferma.</p>}
                  ...
                  {!isRequestConfirmed ? (
                    <button type="button" onClick={confirmRequest} disabled={!privacyAccepted || isSubmittingRequest} className="btn mt-5 border-none bg-brand-blue font-bold text-white hover:bg-blue-700 disabled:bg-slate-300">{isSubmittingRequest ? 'Invio in corso…' : 'Avvia diagnosi preliminare'}</button>
                  ) : (
                    <div className="mt-6">
                ...
                <motion.p key="thanks-waiting-phone" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-5 text-sm leading-relaxed text-slate-500">Inserisci il cellulare per ricevere il codice sconto.</motion.p>
              ) : (
        ...
        <p className="mt-5 max-w-3xl text-sm leading-relaxed text-slate-500">La visura osserva segnali pubblici. Non accede a traffico, vendite, Analytics o Search Console senza autorizzazione.</p>
      </div>
    </section>
  );
}

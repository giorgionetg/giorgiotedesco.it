...
export default function onCryptocurrencies({ postListCryptocurrencies, categoryList }) {
  ...
  return (
    <div>
      ...
      <Layout title='On Web Development' description='Understanding Websites works' image='dmitry-demidko-OG3A-ilG8AY-unsplash.jpg'>
        ...
        <GridContainer className={classes.gridStyle}>
          ...
          {postListCryptocurrencies.map((item) => {
            let linking = '/the-blog/on-web-development/' + item.slug;
...
export async function getStaticProps(context) {
  ...
  let posts = await mdLoader('on-web-development');
  let paths = [];

  ...
  return (
    <div>
      ...
      <Layout
        ...
        description="Understanding Bitcoin, Ethereum and Xrp"
      ...
      >
        <GridContainer className={classes.gridStyle}>
          ...
          {postListCryptocurrencies.map((item) => {
            const linking = `/the-blog/on-cryptocurrencies/${item.slug}`;
...
export async function getStaticProps() {
  const posts = await mdLoader('on-cryptocurrencies');
  const paths = [];
  ...
  const categoryList = [
    ...
    {
      slug: 'on-artificial-intelligence',
      title: 'The Blog on Artificial Intelligence',
    },
  ];
  ...
  for (let p = 0; p < paths.length; p += 1) {
    const data = await mdCryptoLoader(`on-cryptocurrencies/${paths[p]}.md`);

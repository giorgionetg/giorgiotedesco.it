...
export default function SinglePost({ seoData, seoUrl, articleContent }) {
  ...
  const url = `https://www.giorgiotedesco.it/the-blog/on-cryptocurrencies/${seoUrl}`;
  return (
    <div>
      <Header
      ...
      <NextSeo title={seoTitle} description={seoDescription} canonical={url} />
      <ArticleJsonLd
        url={url}
        title={seoTitle}
...
export async function getStaticPaths() {
  const post = await mdLoader('on-cryptocurrencies');
  const paths = [];
...
export async function getStaticProps(context) {
  const data = await mdCryptoLoader(
    `on-cryptocurrencies/${context.params.slug}.md`,
  );
